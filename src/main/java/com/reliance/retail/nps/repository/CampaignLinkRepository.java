...
//    Optional<List<Map<Long,Long>>>findCountByOccurrence();
    @Query(value = "SELECT campaign.name, campaign.id, SUM(campaign_link.campaign_id) FROM campaign JOIN campaign_link ON campaign.id=campaign_link.campaign_id GROUP BY campaign.id" , nativeQuery = true)
